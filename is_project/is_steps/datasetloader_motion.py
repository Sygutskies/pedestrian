...
class DatasetLoaderMotion(BaseDatasetLoader):
    def __init__(self, dir_path:str = "pedestrian/is_project/datasets_csv/", 
        ...
        super().__init__(dir_path, max_frames_number, class_2_int, prefix, postfix, phone, split, train_images_percent, model_type="motion")
    ...
    def run(self):
        ...
        data_y_motion = np.array(tf.keras.utils.to_categorical(data_y_motion, num_classes=3))
        if self.split:
            X_train, X_test, y_train, y_test = train_test_split(data_x, data_y_motion, test_size= 1 - self.train_images_percent)
            ...
            y = tf.convert_to_tensor(data_y_motion, dtype=tf.float32)
