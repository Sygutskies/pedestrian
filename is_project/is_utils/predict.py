...
def run(poseweights:str = "yolov8s-pose.pt", lstm_weights:str = "tmp/checkpoint", video_source:str="pedestrian/is_project/test_movies/crowd1.webm"):

    model = YOLO(poseweights)
    results = model.track(video_source, persist=True)
    import tensorflow as tf
    with tf.device("/cpu:0"):    
        lstm_model = PhoneModel()
        lstm_model.build(input_shape=(None, 10, 29))

        lstm_model.compile(optimizer='Adam', loss='categorical_crossentropy', metrics=['categorical_accuracy'])
        lstm_model.load_weights("logs/2024114177/weights/best/checkpoint")    
        rows = []
        counter = 0 
